...
export default function HomePage() {
  ...
  const openLocker = () => {
    console.log('Button clicked - attempting to open locker');
    if (typeof (window as any).triggerLocker === 'function') {
      (window as any).triggerLocker();
    } else {
      console.error('triggerLocker function not available');
      // Fallback - try direct call
      if (typeof (window as any)._Ht === 'function') {
        console.log('Using fallback _Ht call');
        (window as any)._Ht();
      } else {
        console.error('_Ht function not available');
